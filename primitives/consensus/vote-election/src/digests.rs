...
{
	fn ve_seal(signature: Signature) -> Self {
		DigestItem::Seal(VOTE_ENGINE_ID, signature.encode())
	}

	fn as_ve_seal(&self) -> Option<Signature> {
		self.seal_try_to(&VOTE_ENGINE_ID)
	}
	...
	// }
	fn ve_pre_digest(digest: PreDigest) -> Self {
		DigestItem::PreRuntime(VOTE_ENGINE_ID, digest.encode())
	}
	...
	// }
	fn as_ve_pre_digest(&self) -> Option<PreDigest> {
		self.pre_runtime_try_to(&VOTE_ENGINE_ID)
	}
}
