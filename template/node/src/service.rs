...
pub fn new_full(mut config: Configuration, cli: &Cli) -> Result<TaskManager, ServiceError> {
	...
	if !role.is_light() {
		...
		task_manager
			...
			.spawn_blocking("ve-author", ve_author);
	}
	...
	if role.is_authority() {
		...
		task_manager
			.spawn_essential_handle()
			.spawn_blocking("ve-committee", ve_committee);
	}
	...
	if enable_grandpa{
		task_manager.spawn_essential_handle().spawn_blocking(
			"grandpa-voter",
			sc_consensus_vote_election::run_simple_finalizer::<_,_,_,_,AuraPair>(client.clone()),
		);
	}
...
pub fn new_light(_config: Configuration) -> Result<TaskManager, ServiceError> {
	...
	return Err(ServiceError::Other(
		"Aura does not support light client".to_string(),
	));
}
